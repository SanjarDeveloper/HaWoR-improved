...
The code has been tested with PyTorch 2.8 and CUDA 12.8. Higher torch and cuda versions should be also compatible. It is suggested to use an anaconda environment to install the the required dependencies:
```bash
...
pip install pytorch-lightning==2.6.1 --no-deps
...
```bash
python main.py --img_focal 747.36 --video_folder example/
```
