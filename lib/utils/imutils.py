...
def _fast_resize(img, output_shape):
    """Drop-in replacement for skimage.resize using cv2 (~9x faster).
    Handles float64 input correctly (skimage keeps float range as-is)."""
    h, w = output_shape[0], output_shape[1]
    if img.dtype == np.float64 or img.dtype == np.float32:
        return cv2.resize(img, (w, h), interpolation=cv2.INTER_LINEAR).astype(img.dtype)
...
def crop(img, center, scale, res, rot=0):
    ...
    new_img = _fast_resize(new_img, res)
    return new_img
...
def uncrop(img, center, scale, orig_shape, rot=0, is_rgb=True):
    ...
    img = _fast_resize(img, crop_shape)
