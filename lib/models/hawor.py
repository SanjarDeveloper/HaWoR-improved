...
class HAWOR(pl.LightningModule):
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        # --- Mixed-precision region: backbone + transformers ---
        with torch.amp.autocast('cuda', enabled=image.is_cuda):
            # backbone
        ...
        # --- fp32 from here: MANO skinning + projection need full precision ---
        pred_pose = pred_pose.float()
        pred_shape = pred_shape.float()
        pred_cam = pred_cam.float()
        pred_rotmat_0 = pred_rotmat_0.float()
        ...
        output = {
            'pred_mano_params': {
                'global_orient': out['pred_rotmat'][:, :1],
                'hand_pose': out['pred_rotmat'][:, 1:],
                'betas': out['pred_shape'],
            },
            'pred_keypoints_3d': j3d,
    ...
    def inference(self, imgfiles, boxes, img_focal, img_center, device='cuda', do_flip=False, progress_cb=None):
        ...
        # Use DataLoader with workers for parallel I/O
        loader = DataLoader(db, batch_size=16, shuffle=False,
        ...
        for batch_idx, batch in enumerate(tqdm(loader, desc="HaWoR inference")):
            batch_len = batch['img'].shape[0]

            # Pad incomplete last batch to 16
            if batch_len < 16:
                pad = 16 - batch_len
            ...
            with torch.no_grad():
                batch = {k: v.to(device, non_blocking=True).unsqueeze(0)
                         for k, v in batch.items() if isinstance(v, torch.Tensor)}
                output = self.forward(batch)
                out = output['out']

            # Trim padding from last batch
            if batch_len < 16:
                out = {k: v[:batch_len] for k, v in out.items()}
